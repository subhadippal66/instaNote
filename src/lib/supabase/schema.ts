...
export const FileShared = pgTable('fileShared',{
    userIdOwner: uuid('userIdOwner').notNull().references(()=>users.id,{onDelete: 'cascade'}),
    createdAt: timestamp('created_at', {
        withTimezone: true,
        mode: 'string',
    }).defaultNow().notNull(),
    fileId: uuid('fileId').notNull().references(()=>files.id,{onDelete: 'cascade'}),
    userIdShared: uuid('userIdShared').notNull().references(()=>users.id,{onDelete: 'cascade'}),
